...
main (int argc, char **argv)
{
  ...
  // Routing header data: Type 3 - Source Routing Header for RPL (RFC 6554)
  ...
  // First address: you need to fill this out
  ...
  c += 16;
  // Second address: you need to fill this out
  ...
  memcpy (route_data + c, tmp, 16 * sizeof (uint8_t));
  c += 16;
  // Third address: you need to fill this out
  strcpy (address, "2001:db8::65ff:203c:9901:ab2b");
  if ((status = inet_pton (AF_INET6, address, &sa.sin6_addr)) != 1) {
    fprintf (stderr, "inet_pton() failed.\nError message: %s", strerror (status));
    return (EXIT_FAILURE);
  }
  ...
  for (i=0; i<nframes; i++) {
    ...
    // Copy routing header data (type 2) to ethernet frame.
