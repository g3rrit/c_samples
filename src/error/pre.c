...
result loo()
{
    { struct result_t *_res = malloc(sizeof(struct result_t)); _res->is_error = 1; struct error_t *_err = malloc(sizeof(struct error_t)); _err->msg = "loo error"; _err->err_h = &loo_err_h; _res->err = _err; _res->err->prev_err = 0; return _res; };
    int res = 0;
    { struct result_t *_res = malloc(sizeof(struct result_t)); _res->is_error = 0; _res->ok = res; return _res; };
}
...
{
    int res = 0;
    { if(loo()->is_error) { struct error_t *_n_err = malloc(sizeof(struct error_t)); _n_err->msg = "loo error in foo"; _n_err->err_h = &foo_err_h; _n_err->prev_err = loo()->err; loo()->err = _n_err; return loo(); } else { res = loo()->ok; } };
    { struct result_t *_res = malloc(sizeof(struct result_t)); _res->is_error = 0; _res->ok = res; return _res; };
}
...
{
    int res = 0;
    { if(foo()->is_error) { struct error_t *_n_err = malloc(sizeof(struct error_t)); _n_err->msg = "foo error in bar"; _n_err->err_h = &bar_err_h; _n_err->prev_err = foo()->err; foo()->err = _n_err; return foo(); } else { res = foo()->ok; } };
    { struct result_t *_res = malloc(sizeof(struct result_t)); _res->is_error = 0; _res->ok = res; return _res; };
}
...
{
    int res = 0;
    { if(bar()->is_error) { struct error_t *_err = bar()->err; while(_err) { void (*err_h)() = _err->err_h; char *msg = _err->msg; { int res = 0; err_h(&res); printf("error msg: %s res: %i\n", msg, res); }; struct error_t *_f_err = _err; _err = _err->prev_err; free(_f_err); } free(bar()); } else { res = bar()->ok; } };
